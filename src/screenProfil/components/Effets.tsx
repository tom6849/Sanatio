...
const Effets = () => {
  ...
  const handleAddEffet = () => {
    if (effet.trim() !== '') {
      ...
      setEffet("");
      addBdEffects(newEffetsList);
    }
  };

  const addBdEffects = async (newEffetsList: Effet[]) => {
    try {
  ...
  const loadingEffects = async () => {
    try {
      ...
      if (storedEffect) {
        setEffetsList(JSON.parse(storedEffect));
      }
    } catch (error) {
      console.error('Erreur pour recuperer les données', error);
    }
  };
